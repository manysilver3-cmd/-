...
4. 물, 생수, 탄산수, 보리차, 녹차, 제로콜라, 제로사이다 등 칼로리가 없거나 무의미한 음료/수분은 반드시 0 kcal로 계산하세요.
5. 아래 제공된 [브랜드별 메뉴 칼로리 DB]에 매칭되는 음식이 있다면 반드시 해당 정확한 명칭과 칼로리(kcal)를 사용하세요.
6. 반드시 유효한 JSON 문자열만 출력하세요.
