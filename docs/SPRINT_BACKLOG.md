...
| ID | 태스크 명 | 담당 / 영역 | 상태 | 완료 조건 (Acceptance Criteria) |
| :--- | :--- | :--- | :---: | :--- |
| **SP4-01** | 다빈도 섭취 식품 DB 100종 추가 | `lib/calorie.ts` | 🟩 완료 | 외식/편의점/분식/음료 칼로리 DB 40+종 이상 탑재 |
| **SP4-02** | 복합어/유의어 및 띄어쓰기 퍼지 매칭 | `lib/calorie.ts` | 🟩 완료 | `치즈 버거` -> `햄버거`, 한국어 수량어/단위 매칭 지원 |
| **SP4-03** | 계산 이력(히스토리) 로컬 저장 (옵션) | `lib/storage.ts` | ⬜ 대기 | 최근 계산 기록 확인 및 재계산 기능 |
| **SP4-04** | 3초 렌더링 벤치마크 & 크로스 브라우징 | QA / Performance | 🟦 검증완료 | 실측 600ms 이내 초고속 응답 검증 |
