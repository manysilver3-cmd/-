...
export const FOOD_DB: Record<string, number> = {
  ...
  // ─── 편의점 삼각김밥 (Convenience Store Triangle Kimbap) ───
  삼각김밥: 190,
  참치마요삼각김밥: 210,
  참치마요: 210,
  참치삼각김밥: 200,
...
export const WORKOUTS: Array<{
  ...
  { key: 'swim', label: '수영', perMin: 9, kcalPerHour: 540, intensity: '자유형 중간 속도', category: 'outdoor' },
  { key: 'homet', label: '홈트 / 인터벌', perMin: 8.5, kcalPerHour: 510, intensity: '전신 피트니스', category: 'indoor' },
  { key: 'hiking', label: '등산', perMin: 9, kcalPerHour: 540, intensity: '중급 산행 코스', category: 'outdoor' },
  { key: 'badminton', label: '배드민턴', perMin: 6, kcalPerHour: 360, intensity: '게임 / 랠리', category: 'outdoor' },
  { key: 'tennis', label: '테니스', perMin: 8.5, kcalPerHour: 510, intensity: '스트로크 / 매치', category: 'outdoor' },
  { key: 'boxing', label: '복싱 / 미트', perMin: 10, kcalPerHour: 600, intensity: '고강도 스파링', category: 'intense' },
  { key: 'pilates', label: '필라테스 / 요가', perMin: 4.5, kcalPerHour: 270, intensity: '코어 유연성', category: 'indoor' },
]
...
export const BRAND_MENU_DB: Record<string, Record<string, { kcal: number; label: string }>> = {
  ...
  // ── 배달앱 인기 브랜드 (Baemin, Yogiyo) ──
  지코바: {
    양념치킨: { kcal: 1800, label: '지코바 양념치킨' },
    순살양념치킨: { kcal: 1750, label: '지코바 순살양념치킨' },
    소금구이: { kcal: 1400, label: '지코바 소금구이치킨' },
  },

  네네치킨: {
    스노윙치킨: { kcal: 2300, label: '네네치킨 스노윙치킨' },
    오리엔탈파닭: { kcal: 2400, label: '네네치킨 오리엔탈파닭' },
    후라이드: { kcal: 2000, label: '네네치킨 후라이드' },
    쇼킹핫: { kcal: 2350, label: '네네치킨 쇼킹핫 양념치킨' },
  },

  호식이두마리치킨: {
    후라이드: { kcal: 1950, label: '호식이 후라이드 1마리' },
    간장치킨: { kcal: 2050, label: '호식이 간장치킨 1마리' },
    양념치킨: { kcal: 2150, label: '호식이 양념치킨 1마리' },
    매운간장치킨: { kcal: 2100, label: '호식이 매운간장치킨 1마리' },
  },

  동대문엽기떡볶이: {
    엽기떡볶이: { kcal: 3500, label: '엽떡 엽기떡볶이 (1통)' },
    엽기오뎅: { kcal: 3200, label: '엽떡 엽기오뎅 (1통)' },
    로제떡볶이: { kcal: 4200, label: '엽떡 로제떡볶이 (1통)' },
    마라떡볶이: { kcal: 4500, label: '엽떡 마라떡볶이 (1통)' },
    주먹김밥: { kcal: 450, label: '엽떡 주먹김밥' },
    모둠튀김: { kcal: 650, label: '엽떡 모둠튀김' },
  },

  신전떡볶이: {
    떡볶이: { kcal: 450, label: '신전떡볶이 (1인분)' },
    로제떡볶이: { kcal: 650, label: '신전 로제떡볶이 (1인분)' },
    치즈떡볶이: { kcal: 550, label: '신전 치즈떡볶이 (1인분)' },
    오뎅튀김: { kcal: 220, label: '신전 오뎅튀김 (6개)' },
    참치마요컵밥: { kcal: 520, label: '신전 참치마요컵밥' },
    치즈김밥: { kcal: 480, label: '신전 치즈김밥' },
  },

  배떡: {
    로제떡볶이: { kcal: 2500, label: '배떡 로제떡볶이 (1~2인분)' },
    분모자떡볶이: { kcal: 2600, label: '배떡 분모자 로제떡볶이' },
    마라로제떡볶이: { kcal: 2800, label: '배떡 마라로제떡볶이' },
  },

  죠스떡볶이: {
    떡볶이: { kcal: 430, label: '죠스떡볶이 (1인분)' },
    수제튀김: { kcal: 380, label: '죠스 수제튀김' },
    순대: { kcal: 350, label: '죠스 진짜찰순대' },
  },

  이삭토스트: {
    햄스페셜: { kcal: 420, label: '이삭 햄스페셜 토스트' },
    베이컨베스트: { kcal: 450, label: '이삭 베이컨베스트 토스트' },
    딥치즈베이컨: { kcal: 510, label: '이삭 딥치즈베이컨 토스트' },
    햄치즈: { kcal: 380, label: '이삭 햄치즈 토스트' },
    피자토스트: { kcal: 470, label: '이삭 피자 토스트' },
  },

  명랑핫도그: {
    명랑핫도그: { kcal: 340, label: '명랑핫도그' },
    반반핫도그: { kcal: 380, label: '명랑 모짜렐라인더핫도그 (반반)' },
    통모짜핫도그: { kcal: 410, label: '명랑 통모짜핫도그' },
    감자핫도그: { kcal: 490, label: '명랑 감자핫도그' },
    고구마통모짜: { kcal: 520, label: '명랑 고구마통모짜핫도그' },
  },
}
